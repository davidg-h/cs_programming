...
class ShopEngine
{
  ...
  public function __construct()
  {
    ...
    try {
      $this->pdo = new PDO("sqlite:X:\\1.SchuleTHSynchro\\MinSemester4\\WebProg\\Uebungen\\ToyModels_WebShop\\MVC_Modell\\toymodels.db");
  ...
  public function getMainContentData($filter = "")
  {
    ...
    foreach ($resultData as $row) {
      $button = "<form action='index.php' method='GET'><input hidden name='artNo' value='{$row["ArtikelNr"]}'><button type='submit'>In den Warenkorb</button></form>";
      $item = [$row["ArtikelNr"], $row["ArtikelName"], $row["Beschreibung"], $row["Listenpreis"] . " &euro;", $button];
      $result["content"][] = $item;
    }
  ...
  public function getCartContentData()
  {
    ...
    $grandTotal = 0;
    foreach ($rows as $row) {
      $item = [$row["ArtikelNr"], $row["ArtikelName"], $this->cart[$row["ArtikelNr"]], $row["Listenpreis"] . " &euro;", $row["Listenpreis"] * $this->cart[$row["ArtikelNr"]] . " &euro;"];
      $result["content"][] = $item;
      $grandTotal += $row["Listenpreis"] * $this->cart[$row["ArtikelNr"]];
    }
    $result["content"][] = ["Gesamsumme", "", "", "", $grandTotal . " &euro;"];
